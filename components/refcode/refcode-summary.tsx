...
export function RefCodeSummary() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">จำนวนเดือน</CardTitle>
          <Calendar className="h-4 w-4 text-primary" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{monthCount}</div>
          <p className="text-xs text-muted-foreground">เดือนที่มีข้อมูล</p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">รายการ/เดือน</p>
        </CardContent>
      </Card>
    </div>
  )
